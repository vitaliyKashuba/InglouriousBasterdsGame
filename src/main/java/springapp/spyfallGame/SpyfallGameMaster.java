...
public class SpyfallGameMaster extends BasicGameMaster {
    ...
    private SpyfallGameMaster() {
        ...
        try {
            String s =
                    AppUtil.readFromResources("spyfall.json");                                          // load locations and roles from resources
    ...
    public void startGame(int adminId) {
        ...
        Player spy =
                Randomizer.getRandomElement(players);                                                              // select spy
        ...
        List<String> rls =
                locationsAndRoles.get(location);                                                             // get location roles
        ...
        for (Player p : players)    // TODO send img with location ?
        {
            ...
            messageSender.sendMesageToUser(p,
                                           locationsMessage);                                                        // send locations list
            messageSender.sendMesageToUser(p,
                                           roleMessage);                                                             // send role
        }
    }
    ...
    public void setLocationsLimit(int adminId, int limit) {
        ...
        int roomId = getRoomIdByAdminId(adminId);
        roomLocationsLimit.put(roomId, Math.min(limit, locationsAndRoles.keySet().size()));
    }
